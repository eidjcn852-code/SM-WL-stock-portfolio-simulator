(() => {
      ...
      function refreshDriveControls(updateStatus = true) {
        ...
        if (storageMode) storageMode.textContent = connected ? '本機自動儲存＋Drive 手動備份' : '本機自動儲存';

        if (updateStatus) {
          if (!available) setDriveStatus('Google Drive 模組無法載入', true);
          else if (connected) setDriveStatus('已連接 ' + Drive.ACCOUNT_EMAIL + '，按「儲存雲端」才會更新備份', false);
      ...
      async function connectDrive() {
        ...
        try {
          ...
          setDriveStatus('已連接 ' + connection.accountEmail + '，按「儲存雲端」才會更新備份', false);
          return true;
        } catch (error) {
